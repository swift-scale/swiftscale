...
export default function AdminLogin() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push("/admin");
      router.refresh();
      // On success, we don't set loading to false to keep the screen locked during redirect
    } catch (error: any) {
    ...
    <div className="admin-theme min-h-screen relative flex items-center justify-center bg-background text-foreground selection:bg-primary selection:text-primary-foreground overflow-hidden font-sans">
      ...
      <div className="relative z-20 w-full max-w-5xl px-6 animate-in fade-in zoom-in-95 duration-1000">
              ...
              <div className="relative z-10">
                ...
                <div className="space-y-6">
                  ...
                  <p className="text-foreground/50 text-lg leading-relaxed max-w-xs font-medium">
                    Experience the next generation of infrastructure management.
                    Secure, scalable, and surgical.
                  </p>
                </div>
              </div>
              ...
              <div className="max-w-sm mx-auto space-y-8">
                <div className="space-y-2">
                  ...
                  <p className="text-foreground/50 font-medium">
                    Access your administrative board
                  </p>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  <div className="space-y-4">
                    <div className="space-y-2">
                      <Label
                        htmlFor="email"
                        className="text-[10px] font-black uppercase tracking-[0.2em] text-foreground/40 ml-1"
                      >
                        Admin Identity
                      </Label>
                  ...
                  <div className="flex items-center space-x-3 ml-1">
                    <Checkbox
                      id="remember"
                      className="w-5 h-5 border-border/60 data-[state=checked]:bg-primary data-[state=checked]:border-primary rounded-lg"
                    />
                    <label
                      htmlFor="remember"
                  ...
                  >
                    <div
                      className={`flex items-center justify-center gap-3 transition-transform duration-500 ${isLoading ? "-translate-y-16" : ""}`}
                    >
                      AUTHENTICATE{" "}
                      <ArrowRight className="w-6 h-6 group-hover:translate-x-1 transition-transform" />
                    </div>
                    <div
                      className={`absolute inset-0 flex items-center justify-center gap-3 transition-transform duration-500 ${isLoading ? "translate-y-0" : "translate-y-16"}`}
                    >
        ...
        <div className="mt-8 text-center">
          <Link
            href="/"
            className="text-foreground/40 hover:text-primary text-xs font-black uppercase tracking-[0.3em] transition-all"
          >
